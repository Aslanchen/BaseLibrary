...
public class FlexibleItemDecoration extends RecyclerView.ItemDecoration {
   ...
   *
   * @param viewTypes the specific ViewTypes for which the divider will be drawn OR do not specify any to draw divider for all ViewTypes (default
   *                  behavior).
  ...
  public FlexibleItemDecoration withDrawDividerOnLastItem(boolean lastItem) {
    if (lastItem) {
      mDividerOnLastItem = 0;
    } else {
      mDividerOnLastItem = 1;
    }
    return this;
  }
   ...
   * thus appear <i>over</i> the views.</p> Default value is false (drawn underneath).
   *
  ...
  public FlexibleItemDecoration withSectionGapOnLastItem(boolean lastItem) {
    if (lastItem) {
      mSectionGapOnLastItem = 1;
    } else {
      mSectionGapOnLastItem = 0;
    }
    return this;
  }
  ...
  /**
   * As {@link #addItemViewType(int)} but with custom offset equals to all sides that will affect only this viewType.
   *
   * @param viewType the viewType affected
   * @return this FlexibleItemDecoration instance so the call can be chained
   * @see #withOffset(int)
  ...
  public void getItemOffsets(final Rect outRect, View view, RecyclerView recyclerView, RecyclerView.State state) {
    ...
    } else if (recyclerView.getLayoutManager() instanceof FlexibleFlexboxLayoutManager) {
      orientation = ((FlexibleFlexboxLayoutManager) recyclerView.getLayoutManager()).getOrientation();
    }
