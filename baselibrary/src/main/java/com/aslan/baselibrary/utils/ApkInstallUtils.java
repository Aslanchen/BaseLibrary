...
import com.aslan.baselibrary.utils.ShellUtils.CommandResult;
import java.io.File;
...
public class ApkInstallUtils {
   ...
   * @return
   */
  public static boolean install(Context context, String authority, File apkFile) throws IOException {
    return install(context, authority, apkFile.getCanonicalPath());
  }
  ...
  public static boolean install(Context context, String authority, String filePath) {
    if (ApkInstallUtils.isSystemApplication(context)
        || ShellUtils.checkRootPermission()) {
      return installAppSilent(context, filePath);
    }
    return installNormal(context, authority, filePath);
  }
  ...
  private static boolean installAppSilentBelow24(Context context, String filePath) {
    ...
    if (!FileUtil.isFileExists(context, file)) {
      return false;
    }
   ...
   */
  public static boolean installNormal(Context context, String authority, String filePath) {
    File file = FileUtil.getFileByPath(filePath);
    if (file == null) {
      return false;
    }
    return FileUtil.isFileExists(context, file) && installNormal(context, authority, file);
  }
   ...
   * @return 安装是否成功
   */
  public static boolean installNormal(Context context, String authority, File appFile) {
    try {
      Intent intent = getInstallAppIntent(context, authority, appFile);
   ...
   * @return
   */
  public static Intent getInstallAppIntent(Context context, String authority, File appFile) {
    try {
      ...
      Uri fileUri = FileUtil.getUriByFile(context, authority, appFile);
